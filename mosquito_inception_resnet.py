...
def MakeLabelSet(RawLevelList,LevelDir):
	...
	with open('mosquito_raw_2.csv','w') as csvfile:
		wr = csv.writer(csvfile)
		wr.writerow(WholeLevelList)
...
def MakeStemLayer(X): # X is tf.placeholder in main function
	...
	W1 = tf.layers.conv2d(inputs=X,filters=8,strides=[2,2],kernel_size=[3,3],padding="VALID",activation = tf.nn.relu)
	L1 = tf.layers.dropout(inputs=W1,rate=0.7,training = True)

	W2 = tf.layers.conv2d(inputs=L1,filters=8,strides=[1,1],kernel_size=[3,3],padding="VALID",activation=tf.nn.relu)
	L2 = tf.layers.dropout(inputs=W2,rate=0.7,training = True)

	W3 = tf.layers.conv2d(inputs=L2,filters=16,strides=[1,1],kernel_size=[3,3],padding="SAME",activation=tf.nn.relu)
	...
	W4_2 = tf.layers.conv2d(inputs=L3,filters=24,kernel_size=[3,3],strides=[2,2],padding="VALID",activation=tf.nn.relu)
...
def Inception_4A(X):
	...
	W1_3 = tf.layers.conv2d(inputs=X,filters=16,kernel_size=[1,1],strides=[1,1],padding="SAME",activation=tf.nn.relu)
	L1_3 = tf.layers.dropout(inputs=W1_3,rate=0.7,training=True)

	W1_4 = tf.layers.conv2d(inputs=X,filters=16,kernel_size=[1,1],strides=[1,1],padding="SAME",activation=tf.nn.relu)
	L1_4 = tf.layers.dropout(inputs=W1_4,rate=0.7,training=True)

	W2_1 = tf.layers.conv2d(inputs=L1_1,filters=24,kernel_size=[1,1],strides=[1,1],padding="SAME",activation=tf.nn.relu)
	L2_1 = tf.layers.dropout(inputs=W2_1,rate=0.7,training=True)

	W2_2 = tf.layers.conv2d(inputs=L1_3,filters=24,kernel_size=[3,3],strides=[1,1],padding="SAME",activation=tf.nn.relu)
	L2_2 = tf.layers.dropout(inputs=W2_2,rate=0.7,training=True)

	W2_3 = tf.layers.conv2d(inputs=L1_4,filters=24,kernel_size=[3,3],strides=[1,1],padding="SAME",activation=tf.nn.relu)
	L2_3 = tf.layers.dropout(inputs=W2_3,rate=0.7,training=True)

	W3 = tf.layers.conv2d(inputs=L2_3,filters=24,kernel_size=[3,3],strides=[1,1],padding="SAME",activation=tf.nn.relu)
	L3 = tf.layers.dropout(inputs=W3,rate=0.7,training=True)
...
def Reduction_A(X):
	...
	W1 = tf.layers.conv2d(inputs=X,filters=48,kernel_size=[1,1],strides=[1,1],padding="SAME",activation=tf.nn.relu)
	L1 = tf.layers.dropout(inputs=W1,rate=0.7,training=True)

	W2_1 = tf.layers.max_pooling2d(inputs=X,pool_size=[3,3],strides=[2,2],padding="VALID")
	L2_1 = tf.layers.dropout(inputs=W2_1,rate=0.7,training=True)

	W2_2 = tf.layers.conv2d(inputs=X,filters=24,kernel_size=[3,3],strides=[2,2],padding="VALID",activation=tf.nn.relu)
	L2_2 = tf.layers.dropout(inputs=W2_2,rate=0.7,training=True)

	W2_3 = tf.layers.conv2d(inputs=W1,filters=56,kernel_size=[3,3],strides=[1,1],padding="SAME",activation=tf.nn.relu)
	L2_3 = tf.layers.dropout(inputs=W2_3,rate=0.7,training=True)

	W3 = tf.layers.conv2d(inputs=L2_3,filters=64,kernel_size=[3,3],strides=[2,2],padding="VALID",activation=tf.nn.relu)
	L3 = tf.layers.dropout(inputs=W3,rate=0.7,training=True)
...
def Inception_7B(X):
	...
	W2_1 = tf.layers.conv2d(inputs=L1_1,filters=32,kernel_size=[1,1],strides=[1,1],padding="SAME",activation=tf.nn.relu)
	L2_1 = tf.layers.dropout(inputs=W2_1,rate=0.7,training=True)

	W2_2 = tf.layers.conv2d(inputs=X,filters=96,kernel_size=[1,1],strides=[1,1],padding="SAME",activation=tf.nn.relu)
	L2_2 = tf.layers.dropout(inputs=W2_2,rate=0.7,training=True)

	W2_3 = tf.layers.conv2d(inputs=L1_2,filters=56,kernel_size=[1,7],strides=[1,1],padding="SAME",activation=tf.nn.relu)
	L2_3 = tf.layers.dropout(inputs=W2_3,rate=0.7,training=True)

	W2_4 = tf.layers.conv2d(inputs=L1_3,filters=48,kernel_size=[1,7],strides=[1,1],padding="SAME",activation=tf.nn.relu)
	L2_4 = tf.layers.dropout(inputs=W2_4,rate=0.7,training=True)

	W3_1 = tf.layers.conv2d(inputs=L2_3,filters=64,kernel_size=[1,7],strides=[1,1],padding="SAME",activation=tf.nn.relu)
	L3_1 = tf.layers.dropout(inputs=W3_1,rate=0.7,training=True)

	W3_2 = tf.layers.conv2d(inputs=L2_4,filters=56,kernel_size=[7,1],strides=[1,1],padding="SAME",activation=tf.nn.relu)
	L3_2 = tf.layers.dropout(inputs=W3_2,rate=0.7,training=True)

	W4 = tf.layers.conv2d(inputs=L3_2,filters=56,kernel_size=[1,7],strides=[1,1],padding="SAME",activation=tf.nn.relu)
	L4 = tf.layers.dropout(inputs=W4,rate=0.7,training=True)

	W5 = tf.layers.conv2d(inputs=L4,filters=64,kernel_size=[7,1],strides=[1,1],padding="SAME",activation=tf.nn.relu)
...
def Reduction_B(X):
	...
	W2_1 = tf.layers.conv2d(inputs=L1_2,filters=48,kernel_size=[3,3],strides=[2,2],padding="VALID",activation=tf.nn.relu)
	L2_1 = tf.layers.dropout(inputs=W2_1,rate=0.7,training=True)

	W2_2 = tf.layers.conv2d(inputs=L1_3,filters=64,kernel_size=[1,7],strides=[1,1],padding="SAME",activation=tf.nn.relu)
	L2_2 = tf.layers.dropout(inputs=W2_2,rate=0.7,training=True)

	W3 = tf.layers.conv2d(inputs=L2_2,filters=80,kernel_size=[7,1],strides=[1,1],padding="SAME",activation=tf.nn.relu)
	L3 = tf.layers.dropout(inputs=W3,rate=0.7,training=True)

	W4 = tf.layers.conv2d(inputs=L3,filters=80,kernel_size=[3,3],strides=[2,2],padding="VALID",activation=tf.nn.relu)
	L4 = tf.layers.dropout(inputs=W4,rate=0.7,training=True)
...
def Inception_3C(X):
	...
	W1_2 = tf.layers.conv2d(inputs=X,filters=96,kernel_size=[1,1],strides=[1,1],padding="SAME",activation=tf.nn.relu)
	L1_2 = tf.layers.dropout(inputs=W1_2,rate=0.7,training=True)

	W1_3 = tf.layers.conv2d(inputs=X,filters=96,kernel_size=[1,1],strides=[1,1],padding="SAME",activation=tf.nn.relu)
	L1_3 = tf.layers.dropout(inputs=W1_3,rate=0.7,training=True)

	W2_1 = tf.layers.conv2d(inputs=X,filters=64,kernel_size=[1,1],strides=[1,1],padding="SAME",activation=tf.nn.relu)
	L2_1 = tf.layers.dropout(inputs=W2_1,rate=0.7,training=True)

	W2_2 = tf.layers.conv2d(inputs=L1_3,filters=112,kernel_size=[1,3],strides=[1,1],padding="SAME",activation=tf.nn.relu)
	L2_2 = tf.layers.dropout(inputs=W2_2,rate=0.7,training=True)

	W3_1 = tf.layers.conv2d(inputs=L1_1,filters=64,kernel_size=[1,1],strides=[1,1],padding="SAME",activation=tf.nn.relu)
	L3_1 = tf.layers.dropout(inputs=W3_1,rate=0.7,training=True)

	W3_2 = tf.layers.conv2d(inputs=L1_2,filters=64,kernel_size=[1,3],strides=[1,1],padding="SAME",activation=tf.nn.relu)
	L3_2 = tf.layers.dropout(inputs=W3_2,rate=0.7,training=True)

	W3_3 = tf.layers.conv2d(inputs=L1_2,filters=64,kernel_size=[3,1],strides=[1,1],padding="SAME",activation=tf.nn.relu)
	L3_3 = tf.layers.dropout(inputs=W3_3,rate=0.7,training=True)

	W3_4 = tf.layers.conv2d(inputs=L2_2,filters=128,kernel_size=[3,1],strides=[1,1],padding="SAME",activation=tf.nn.relu)
	L3_4 = tf.layers.dropout(inputs=W3_4,rate=0.7,training=True)

	W4_1 = tf.layers.conv2d(inputs=L3_4,filters=64,kernel_size=[3,1],strides=[1,1],padding="SAME",activation=tf.nn.relu)
	L4_1 = tf.layers.dropout(inputs=W4_1,rate=0.7,training=True)

	W4_2 = tf.layers.conv2d(inputs=L3_4,filters=64,kernel_size=[1,3],strides=[1,1],padding="SAME",activation=tf.nn.relu)
	L4_2 = tf.layers.dropout(inputs=W4_2,rate=0.7,training=True)
...
if __name__ == '__main__':
	...
	print(Stem_Data_humidity.shape) #(5,5,40)
...
# 	#인셉션에서는 Reduction B 자체가 모델이므로. softmax적용해서cost값 정해줘야함. (단 [?,1,1,filters]) <- 형태로
	...
	L_first_softmax_temp = tf.reshape(L_first_softmax_temp,[-1,1*1*384])

	W1 = tf.get_variable("W1",shape=[384,96],initializer=tf.contrib.layers.xavier_initializer())
	...
	W3 = tf.get_variable("W3", shape=[48, 8],initializer=tf.contrib.layers.xavier_initializer())
	...
	Dropout_Inception_3C_Data_AveP = tf.reshape(Dropout_Inception_3C_Data_AveP,[-1,1*1*384])

	W4 = tf.get_variable("W4",shape=[384,192],initializer=tf.contrib.layers.xavier_initializer())
	...
	W5 = tf.get_variable("W5",shape=[192,96],initializer=tf.contrib.layers.xavier_initializer())
	b5 = tf.Variable(tf.random_normal([96]))
	L5 = tf.nn.relu(tf.matmul(L4,W5)+b5)
	L5 = tf.nn.dropout(L5, keep_prob=0.7)

	W6 = tf.get_variable("W8", shape=[96, 8],initializer=tf.contrib.layers.xavier_initializer())
	b6 = tf.Variable(tf.random_normal([8]))
	...
	optimizer = tf.train.AdamOptimizer(0.00001).minimize(cost_result)
	...
	for epoch in range(500):
		...
		for i in range(total_batch):
			...
			_, cost_val,tmp = sess.run([optimizer,cost_result,model_second],feed_dict={X1:Batch_Humidity_a,X2:Batch_RainFall_a,X3:Batch_RainFallDay_a,X4:Batch_AvgTemp_a,X5:Batch_MaxTemp_a,X6:Batch_MinTemp_a,
	...
	for i in range(len(LevelSet_Test)):
		prediction = tf.argmax(model_second, 1)
		predict_level.append(sess.run(prediction, feed_dict={X1: Humidity_Data_Test[i].reshape(-1, 30, 30, 1), X2: RainFall_Data_Test[i].reshape(-1,30,30,1),X3: RainFallDay_Data_Test[i].reshape(-1,30,30,1),X4: AvgTemp_Data_Test[i].reshape(-1,30,30,1),X5: MaxTemp_Data_Test[i].reshape(-1,30,30,1),X6: MinTemp_Data_Test[i].reshape(-1,30,30,1),keep_prob: 1}))

	with open('mosquito_predict_2.csv','w') as csvfile:
		wr =csv.writer(csvfile)
		wr.wrtierow(predict_level)
