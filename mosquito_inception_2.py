...
# 	#인셉션에서는 Reduction B 자체가 모델이므로. softmax적용해서cost값 정해줘야함. (단 [?,1,1,filters]) <- 형태로
	...
	W4 = tf.get_variable("W4",shape=[384,96],initializer=tf.contrib.layers.xavier_initializer())
	...
	W6 = tf.get_variable("W8", shape=[48, 8],initializer=tf.contrib.layers.xavier_initializer())
	...
	optimizer = tf.train.AdamOptimizer(0.00002).minimize(cost_result)
	...
	for i in range(100):
		...
		print(sess.run(prediction, feed_dict={X1: Humidity_Data_Test[i].reshape(-1, 30, 30, 1), X2: RainFall_Data_Test[i].reshape(-1,30,30,1),X3: RainFallDay_Data_Test[i].reshape(-1,30,30,1),X4: AvgTemp_Data_Test[i].reshape(-1,30,30,1),X5: MaxTemp_Data_Test[i].reshape(-1,30,30,1),X6: MinTemp_Data_Test[i].reshape(-1,30,30,1),keep_prob: 1}))
